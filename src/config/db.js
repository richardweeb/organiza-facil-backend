...
const dbPath = path_1.default.join(process.cwd(), "data", "organiza.db");
...
`);
    ...
    if (tableExists("salarios") && !tableHasColumn("salarios", "user_id")) {
        safeExec("migrate:salarios add user_id", `
...
`);
    ...
    if (tableExists("categorias") && !tableHasColumn("categorias", "user_id")) {
        safeExec("migrate:categorias add user_id", `
...
`);
    ...
    if (tableExists("gastos") && !tableHasColumn("gastos", "user_id")) {
        safeExec("migrate:gastos add user_id", `
...
`);
    ...
    const anyUser = exports.db.prepare("SELECT id FROM users LIMIT 1").get();
    if (!anyUser) {
        exports.db.prepare("INSERT INTO users (created_at, is_admin, role, plan, preferred_channel) VALUES (?, ?, ?, ?, ?)").run(nowISO(), 1, "admin", "free", "telegram");
    }
    // Conveniência MVP
    exports.db.prepare("UPDATE users SET is_admin = 1 WHERE id = 1").run();
    exports.db.prepare("UPDATE users SET role = 'admin' WHERE id = 1").run();
});
migrate();
// =============================
