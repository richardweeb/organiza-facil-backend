...
async function putJSON(url, body, deps) {
    return fetch(url, {
        method: "PUT",
        headers: {
            "Content-Type": "application/json",
...
function createTelegramBot(token, deps) {
    ...
    bot.command("beneficios", async (ctx) => {
        try {
            ...
            if (plan === "vip") {
                return ctx.reply("💎 VIP ativado!\n\n" +
                    "Você tem:\n" +
                    "• Categorias ilimitadas\n" +
                    "• Exportar gastos em CSV (/export)\n");
            }
            return ctx.reply("🆓 Plano FREE\n\n" +
                "Você tem:\n" +
                "• Até 20 categorias\n" +
                "• Export CSV: VIP\n\n" +
    ...
    bot.command("salario", async (ctx) => {
        try {
            ...
            if (parts.length < 2)
                return ctx.reply("Use assim: /salario 1900");
            ...
            const body = await resp.json().catch(() => ({}));
            if (!resp.ok)
                return ctx.reply(`❌ Erro ao salvar salário: ${body?.error || resp.status}`);
    ...
    bot.command("categoria", async (ctx) => {
        try {
            ...
            const body = await resp.json().catch(() => ({}));
            if (!resp.ok)
                return ctx.reply(`❌ Erro ao criar categoria: ${body?.error || resp.status}`);
    ...
    bot.command("gasto", async (ctx) => {
        try {
            const parts = ctx.message.text.split(" ").filter(Boolean);
            if (parts.length < 4)
                return ctx.reply("Use assim: /gasto 35.90 mercado alimentacao");
            const valor = Number(parts[1].replace(",", "."));
            if (!Number.isFinite(valor) || valor <= 0) {
            ...
            const body = await resp.json().catch(() => ({}));
            if (!resp.ok)
                return ctx.reply(`❌ ${body?.error || "Erro ao salvar gasto"}`);
    ...
    bot.command("resumo", async (ctx) => {
        try {
            ...
            if (sub !== "hoje")
                return ctx.reply("Use assim: /resumo hoje");
            const data = todayISO();
            ...
            const body = await resp.json().catch(() => ({}));
            if (!resp.ok || !body?.ok)
                return ctx.reply(`❌ Erro no resumo: ${body?.error || resp.status}`);
            const r = body.resumo;
            const linhas = [
            ...
            if (!r.ultimos?.length)
                ...
                for (const g of r.ultimos)
                    linhas.push(`- ${formatBRL(g.valor)} • ${g.descricao} • ${g.categoria}`);
    ...
    bot.command("login", async (ctx) => {
        try {
            const parts = ctx.message.text.split(" ").filter(Boolean);
            if (parts.length < 2)
                return ctx.reply("Use assim: /login SEU_CODIGO");
            const code = parts[1].trim().toUpperCase();
            const telegramUserId = ctx.from?.id;
            const telegramUsername = ctx.from?.username;
            if (!telegramUserId)
                return ctx.reply("❌ Não consegui ler seu ID do Telegram.");
    ...
    bot.command("canal", async (ctx) => {
        try {
            ...
            const body = await resp.json().catch(() => ({}));
            if (!resp.ok || !body?.ok)
                return ctx.reply(`❌ ${body?.error || "Falha ao atualizar"}`);
    ...
    async function broadcastViaApi(ctx, message, title) {
        ...
        return { ok: true, recipients: body.recipients, noticeId: body.noticeId };
    }
    bot.command("aviso", async (ctx) => {
        try {
            ...
            if (first === "everyone" || first === "all")
                idx = 2;
            const msg = parts.slice(idx).join(" ").trim();
            if (!msg)
                return ctx.reply("Mensagem vazia.");
    ...
    bot.command("powerremove", async (ctx) => {
        try {
            ...
            const id = (parts[1] || "").trim();
            if (!id)
                return ctx.reply("Use assim: /powerremove ID\n(ID pode ser user_id interno ou Telegram ID)");
    ...
    bot.command("admins", async (ctx) => {
        try {
            ...
            const body = await resp.json().catch(() => ({}));
            if (!resp.ok || !body?.ok)
                return ctx.reply(`❌ ${body?.error || "Falha ao buscar admins"}`);
    ...
    bot.command("mods", async (ctx) => {
        try {
            ...
            const body = await resp.json().catch(() => ({}));
            if (!resp.ok || !body?.ok)
                return ctx.reply(`❌ ${body?.error || "Falha ao buscar moderadores"}`);
            ...
            if (!moderadores.length)
                ...
                lines.push(...moderadores.map(fmt));
            return ctx.reply(lines.join("\n").slice(0, 3800));
        }
        catch (e) {
            console.error(e);
    ...
    bot.command("membros", async (ctx) => {
        try {
            ...
            const labels = { admin: "🛡️ Admin", moderador: "🧰 Moderador", membro: "👤 Membro" };
    ...
    bot.command("ban", async (ctx) => {
        try {
            ...
            if (!id)
                return ctx.reply("Use assim: /ban ID [7d|2h|30m] [motivo]\n(ID pode ser user_id interno ou Telegram ID)");
            ...
            const body = await resp.json().catch(() => ({}));
            if (!resp.ok || !body?.ok)
                return ctx.reply(`❌ ${body?.error || "Falha ao banir"}`);
            const u = body.user;
    ...
    bot.command("unban", async (ctx) => {
        try {
            ...
            const id = (parts[1] || "").trim();
            if (!id)
                return ctx.reply("Use assim: /unban ID\n(ID pode ser user_id interno ou Telegram ID)");
            const url = `${deps.apiBaseUrl}/api/user/unban`;
            const payload = { id, telegramUserId: ctx.from?.id, telegramUsername: ctx.from?.username };
            const resp = await postJSON(url, payload, deps);
            const body = await resp.json().catch(() => ({}));
            if (!resp.ok || !body?.ok)
                return ctx.reply(`❌ ${body?.error || "Falha ao desbanir"}`);
            const u = body.user;
    ...
    bot.command("changelog", async (ctx) => {
        try {
            ...
            if (!msg)
                return ctx.reply("Use assim: /changelog SUA_MENSAGEM");
